...
2. **Zero Cost for End Users:** Consumers download static files from GitHub. No API keys, no runtime, no database.
3. **BYOK for Contributors:** Forkers supply their own keys to regenerate or extend.
4. **Exact Quote Fidelity:** Every YC attribution must be verbatim from source transcripts/articles. No paraphrasing in attribution blocks.
5. **Narrow, Composable Skills:** Each skill covers one specific micro-topic. Skills reference each other via `related_skills` links.
6. **Filter Signals:** `/` = category path filter. `%` = tag filter. Exact resolution algorithm specified in Section 11.
7. **No Runtime RAG:** No vector database in the published product. Matching is pre-computed and embedded in static files.
...
```
Discover -> Download -> Chunk -> Extract -> Cluster -> Synthesize -> Export -> Validate -> Commit -> Tag
```
...
├── data/
│   ├── raw/
│   │   ├── library/              # HTML/Markdown of essays
│   │   └── youtube/              # JSON metadata + VTT transcripts
│   ├── chunks/
│   │   ├── library/              # JSON chunk files
│   │   └── youtube/              # JSON chunk files
│   └── registry.db               # SQLite state machine
├── skills/
│   ├── fundraising/
...
├── .gitignore                    # Must ignore data/raw/, data/chunks/, .env
...
| Embeddings (Local) | `sentence-transformers` | 2.7.x | Clustering + similarity |
...
-- Individual chunks
CREATE TABLE chunks (
    chunk_id TEXT PRIMARY KEY,      -- {content_id}_{index:04d}
...
CREATE TABLE skills (
    ...
    related_skills TEXT,            -- JSON array of skill_ids
...
### 4.2 Content ID Generation (Deterministic)

- **Library essays:** `lib_{slug}` where slug is the last path segment of the URL without extension. Example: `lib_how-to-convince-investors`
- **YouTube videos:** `yt_{video_id}` (11-char YouTube ID)
- **Chunk IDs:** `{content_id}_{index:04d}`. Example: `yt_abc123def45_0003`
...
- If not found, check URL path for known authors (Paul Graham, Sam Altman, etc.)
- If still unknown, set `speaker = NULL`, `designation = NULL` and flag for manual review
...
7. Insert into `content` table with state `downloaded`
...
3. Maximum 5 words after `yc-{category}`
...
Path: `data/chunks/{source_type}/{content_id}_{index:04d}.json`
...
6. Update selected items' state to `extracted` (optimistic lock)
...
6. If multiple speakers disagree on the same topic, note the contradiction.
...
OUTPUT FORMAT: Return a single JSON object with this exact structure:
{
  ...
  "contradictions": [
    {
      "topic": "fundraising",
      "item_ids": ["internal-ref-1", "internal-ref-2"],
...
5. Insert each item into `extracted_items` table
6. Log usage to `usage_log`
...
7. Mark batch state as `clustered`
...
5. List related skill IDs that this skill connects to. Use the taxonomy: yc-{category}-{subcategory}-{descriptor}. If unsure, leave empty array.
...
OUTPUT FORMAT: Return JSON with this exact structure:
{
  ...
  "related_skills": ["yc-other-category-skill-id"],
  "confidence": 0.92
}

RULES:
- skill_id must follow pattern: yc-{category}-{descriptor} (max 5 words after category)
- confidence is a float 0.0-1.0 based on source agreement and quote strength
...
- If the cluster is too weak (sources contradict heavily), set confidence < 0.6 and flag for review.
```
...
7. Write Markdown file (Section 10)
8. Insert into `skills` table with state `draft`
...
provenance:
  ...
  github_run_url: ""
  sources:
    - content_id: "lib_how-to-convince-investors"
...
## 12. Filter & Signal Resolution System
...
The agent loads all `specs/hermes/*.txt` into context. When the user query is received, the agent scans for signal prefixes or computes similarity locally, then injects the matching skill text into the system prompt.
...
To avoid runtime vector DB, similarity is pre-computed at generation time.

**File:** `data/similarity_matrix.json` (committed to repo, regenerated after each batch)
...
3. Compute `rapidfuzz.fuzz.partial_ratio(quote, chunk_text)`
4. Threshold: `>= 85` is PASS. `70-84` is WARNING (flag for human review). `< 70` is FAIL (block commit).
5. If source chunk is not found (e.g., essay not chunked), search in `data/raw/` with same fuzzy logic.
...
5. Check `related_skills` all exist in the repo
...
5. LLM-as-judge (cheap Gemini Flash call): "Does the 'Principle' section introduce any concepts not supported by the verbatim quotes?" -> Must return `{"supported": true}`
...
git add skills/ specs/ data/similarity_matrix.json
...
- Raw YC content: Not redistributed; used under fair use for research/commentary
...
| LLM timeout | Extract | Switch to next provider, retry 2x, then abort batch | Batch state reset to `chunked` |
| JSON parse failure | Extract | Log raw response to `data/errors/{batch_id}.json`, retry with temperature 0.1 | Manual review |
| Cluster size = 1 | Cluster | Item returned to pool, state reset to `chunked` | Will be picked up in future batch |
| Quote verification < 70 | Validate | Skill state set to `failed`, file moved to `skills/_failed/` | Manual fix or regeneration |
...
- [ ] Repository structure matches Section 2 exactly
- [ ] SQLite schema from Section 4.1 is implemented without modification
- [ ] `yt-dlp` wrapper uses exact command from Section 5.2
- [ ] Chunking algorithms match Section 7 parameters exactly
- [ ] Batch size is configurable but defaults to 15, minimum 5
- [ ] Extraction prompt is loaded from Jinja2 template file, not inline string
- [ ] Clustering uses `AgglomerativeClustering` with exact parameters from Section 8.2
- [ ] Synthesis prompt is loaded from Jinja2 template file
- [ ] Skill Markdown format matches Section 10.1 exactly (every section present)
- [ ] Pydantic models match Section 10.2 exactly
...
- [ ] Signal resolver handles `/`, `%`, exact ID, and fuzzy query (Section 12)
- [ ] Similarity matrix is pre-computed and committed (Section 13.1)
- [ ] Quote verifier uses `rapidfuzz` with threshold 85 (Section 14.1)
- [ ] Hallucination guard checks speaker provenance (Section 14.3)
- [ ] GitHub Actions only validates, does NOT generate (Section 14.4)
- [ ] `.gitignore` excludes `data/raw/`, `data/chunks/`, `.env`
- [ ] `requirements.txt` pins exact versions from Section 3
- [ ] CLI entry point is `python -m src.cli` (Section 16.4)
- [ ] BYOK documentation exists at `docs/BYOK.md` (Section 17)
- [ ] All source content is gitignored; only skills and specs are published
