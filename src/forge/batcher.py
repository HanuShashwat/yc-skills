...
def select_batch(
    db_path: str = "data/registry.db", topic: Optional[str] = None, batch_size: int = 15
) -> Tuple[str, List[str]]:
    """
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if len(content_ids) < 5:
                logger.warning(
                    "Only %d items found for topic '%s'. Minimum batch size is 5. Aborting.",
                    len(content_ids),
                    target_topic,
                )
                raise BatchSelectionError(
                    f"Insufficient items for topic '{target_topic}' (found {len(content_ids)}, need 5)."
                )
            ...
            logger.info(
                "Selected batch %s with %d items for topic '%s'",
                batch_id,
                len(content_ids),
                target_topic,
            )
