...
def run_linker(db_path: str = "data/registry.db") -> None:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT skill_id, file_path, name FROM skills WHERE state = 'draft'"
            )
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if not existing_skill_ids:
                ...
                cursor.execute(
                    "SELECT skill_id, file_path, name FROM skills WHERE state != 'synthesized'"
                )
            else:
                placeholders = ",".join(["?"] * len(existing_skill_ids))
                cursor.execute(
                    f"SELECT skill_id, file_path, name FROM skills WHERE skill_id IN ({placeholders})",
                    existing_skill_ids,
                )
    ...
    for new_skill in new_skills:
        ...
        candidates = [
            (sim, sid)
            for i, (sim, sid) in enumerate(zip(similarities, skill_ids))
            if i != idx
        ]
        ...
        for sim, sid in candidates:
            ...
            candidate_file = next(
                (s["file_path"] for s in valid_skills if s["skill_id"] == sid), None
            )
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "UPDATE skills SET related_skills = ?, updated_at = ? WHERE skill_id = ?",
                    (json.dumps(top_3), datetime.now(timezone.utc).isoformat(), new_id),
                )

                # We will update content state to linked at the end of the run
        except sqlite3.Error as e:
            logger.error("Failed to update skill %s in database: %s", new_id, e)
            raise

    # After successfully linking all new skills, update their content state
    try:
        with sqlite3.connect(db_path) as conn:
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE content SET state = 'linked' WHERE state = 'synthesized'"
            )
    except sqlite3.Error as e:
        logger.error("Failed to update content state to linked: %s", e)
        raise
