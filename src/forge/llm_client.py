...
class LLMClient:
    ...
    def _log_usage(self, provider: str, model: str, prompt_tokens: int, completion_tokens: int, 
                   call_type: str, success: bool, error_message: str | None = None, batch_id: str | None = None):
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                cursor.execute(
                    ...
                    """,
                    (provider, model, batch_id, prompt_tokens, completion_tokens, total, 
                     0.0, call_type, timestamp, 1 if success else 0, error_message)
                )
                conn.commit()
        except sqlite3.Error as e:
    ...
    def call(self, prompt: str, call_type: str, temperature: float = 0.3, 
             response_format_json: bool = True, batch_id: str | None = None, 
             estimated_tokens: int = 1000) -> Union[str, Dict]:
        """
            ...
            try:
                ...
                try:
                    ...
                    if response_format_json and "response_format" in str(e):
                        logger.warning("Provider %s does not support JSON response format. Falling back.", provider_name)
                ...
                if response_format_json:
                    try:
                        ...
                        if json_retries_remaining > 0:
                            logger.warning("JSON parse failed, retrying once with temperature=0.1. Error: %s", e)
                            json_retries_remaining -= 1
                            current_temp = 0.1
                            continue
                        else:
                ...
                self._log_usage(provider_name, provider.model, 0, 0, call_type, False, str(e), batch_id)
                if attempt > max_retries:
                    raise RuntimeError(f"Exhausted retries ({max_retries}) for provider {provider_name}. Last error: {e}")
                
                logger.warning("Transient error on %s, retrying (%d/%d)...", provider_name, attempt, max_retries)
