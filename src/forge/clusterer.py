    ...
    for label, item_indices in clusters_dict.items():
        ...
        if len(item_indices) < min_cluster_size:
            ...
            if can_escape:
                ...
                logger.info("Cluster %d (size %d) is using escape hatch (retry >= 3)", label, len(item_indices))
            else:
                # Reject this cluster
                logger.info("Cluster %d (size %d) rejected. Returning items to pool.", label, len(item_indices))
        ...
        db_clusters.append((
            cluster_id, batch_id, topic, summary, len(item_indices), avg_sim, longest_quote, now_iso
        ))
        
        for idx in item_indices:
            db_cluster_items.append((
                cluster_id, items[idx]["item_id"], float(sim_matrix[item_indices[0], idx]) # Just store similarity to the first item for now, or 1.0 if self
            ))
