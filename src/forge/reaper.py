...
def run_reaper(db_path: str = "data/registry.db") -> int:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            logger.info("Reaper found %d stale item(s). Processing...", len(stale_items))
            ...
            for content_id, retry_count, last_processed in stale_items:
                ...
                if new_retry > 3:
                    new_state = 'failed'
                    logger.warning("Item %s failed %d times. Marking as 'failed'.", content_id, new_retry)
                else:
                    new_state = 'chunked'
                    logger.info("Recovering item %s (retry %d/3) to 'chunked' state.", content_id, new_retry)
