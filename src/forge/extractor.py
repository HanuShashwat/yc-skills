...
def run_extraction(batch_id: str, content_ids: List[str], db_path: str = "data/registry.db") -> None:
    """
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            if not rows:
                logger.warning("No chunks found for the given content IDs in batch %s.", batch_id)
                return
    ...
    try:
        response_text = llm.call(prompt=prompt, call_type="extract", temperature=temperature)
        extraction = parse_response(response_text)
    except Exception as e:
        logger.warning("Failed initial extraction parsing for batch %s: %s", batch_id, e)
        if response_text:
        ...
        try:
            response_text = llm.call(prompt=prompt, call_type="extract", temperature=0.1)
            extraction = parse_response(response_text)
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            for item in extraction.extracted_items:
                ...
                if not chunk_id:
                    logger.warning("LLM returned invalid in_batch_index %d. Skipping item.", item.in_batch_index)
                    continue
                ...
                cursor.execute(
                    ...
                    """,
                    (
                        item_id, batch_id, chunk_id, item.in_batch_index, item.quote, item.speaker,
                        item.designation, item.topic, str(item.source_url), is_framework, is_warning, now_iso
                    )
                )
            ...
            logger.info("Successfully extracted %d items for batch %s", len(extraction.extracted_items), batch_id)
