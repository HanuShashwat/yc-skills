"""
Hermes Spec Exporter for YC Skills Forge.
...
def export_hermes(skill_path: str, output_dir: str = "specs/hermes") -> str:
    ...
    when_match = re.search(r"### When to Use This Skill\s+(.+?)(?=### |## |\Z)", body, re.DOTALL)
    ...
    protocol_match = re.search(r"### Agent Protocol\s+(.+?)(?=### |## |\Z)", body, re.DOTALL)
    protocol_text = protocol_match.group(1).strip() if protocol_match else ""
    protocol_steps = [line.strip() for line in protocol_text.split("\n") if re.match(r"^\d+\.", line.strip())]
    protocol_str = "\n".join(protocol_steps)
    
    # Extract Follow-Up Questions
    questions_match = re.search(r"### Follow-Up Questions\s+(.+?)(?=### |## |\Z)", body, re.DOTALL)
...
def export_all_hermes(skills_dir: str = "skills", output_dir: str = "specs/hermes") -> List[str]:
    """
