...
def chunk_transcript(content_id: str, file_path: str, default_speaker: str | None, db_path: str) -> None:
    """
        ...
        if mono_wc <= max_words:
            ...
            for seg in mono:
                ...
                if current_wc >= target_words:
                    ends_with_boundary = bool(re.search(r'[.!?]["\']?\s*$', seg["text"]))
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            conn.commit()
            logger.info("Successfully created %d chunks for %s", len(final_chunks), content_id)
