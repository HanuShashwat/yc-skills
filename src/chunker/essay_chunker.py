...
def chunk_essay(content_id: str, markdown_text: str, speaker: str | None, db_path: str) -> None:
    """
    ...
    for chunk in final_chunks:
        ...
        sentences = get_sentences(chunk)  # Take from original chunk, not overlapped text
        if sentences:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute("UPDATE content SET state = 'chunked' WHERE content_id = ?", (content_id,))
            conn.commit()
            logger.info("Successfully created %d chunks for %s", len(overlapped_chunks), content_id)
