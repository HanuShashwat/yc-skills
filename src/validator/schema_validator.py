"""
Schema Validator for YC Skills Forge.
...
class SchemaValidator:
    ...
    def validate_skill(self, skill_path: str) -> SchemaValidationResult:
        ...
        if path.suffix != ".md":
            ...
            return SchemaValidationResult(skill_id=skill_id, status="fail", errors=errors, warnings=warnings)
        ...
        if frontmatter.skill_id != skill_id:
            errors.append(f"skill_id '{frontmatter.skill_id}' does not match filename '{skill_id}'.")

        # 3. category matches parent directory name
        parent_dir = path.parent.name
        if frontmatter.category != parent_dir:
            errors.append(f"category '{frontmatter.category}' does not match parent directory '{parent_dir}'.")
        ...
        for rel_id in frontmatter.related_skills:
            ...
            if not found:
                errors.append(f"related_skill '{rel_id}' does not correspond to an existing .md file.")
        ...
        return SchemaValidationResult(skill_id=skill_id, status=status, errors=errors, warnings=warnings)
