...
def update_skill_state(db_path: str, skill_id: str, state: str) -> None:
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            cursor.execute(
                "UPDATE skills SET state = ? WHERE skill_id = ?", (state, skill_id)
            )
            conn.commit()
...
def main() -> None:
    ...
    if args.skill_id:
        ...
        skill_files = [
            f for f in skills_dir.rglob("*.md") if f.parent.name != "_failed"
        ]
    ...
    for skill_file in skill_files:
        ...
        schema_status = (
            ...
            else ("FAIL" if schema_res.status == "fail" else "WARN")
        )
        ...
        if schema_res.status == "fail":
            ...
            quote_status = (
                "PASS"
                if quote_res.status == "pass"
                else ("FAIL" if quote_res.status == "fail" else "WARN")
            )
            ...
            if quote_res.status == "fail":
                ...
                if halluc_res.status == "fail":
                    ...
                    quote_status == "WARN"
                    or hallucination_status == "WARN"
                    or schema_status == "WARN"
                ):
