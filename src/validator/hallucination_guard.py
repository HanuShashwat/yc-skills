"""
Hallucination Guard for OpenYC Skills.
...
class HallucinationGuard:
    ...
    def _extract_sections(self, body: str) -> Tuple[str, str, List[Dict]]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith('> "'):
                ...
                quotes.append(
                    {"text": line.lstrip(">").strip().strip('"'), "speaker": speaker}
                )
    ...
    def _call_dedicated_llm(self, prompt: str) -> Optional[Dict]:
        ...
        if provider_name not in self.config.providers.providers:
            logger.error(
                "Dedicated validator provider %s not found in config.", provider_name
            )
            return None
    ...
    def check_skill(self, skill_path: str) -> HallucinationCheckResult:
        ...
        content_ids = [
            s.content_id for s in frontmatter.provenance.sources if s.content_id
        ]
        ...
        prompt = template.render(
            quotes=quotes, principle=principle, application=application
        )
