...
def guess_speaker(description: str) -> Tuple[Optional[str], Optional[str]]:
    ...
    m2 = re.search(r"([A-Z][a-z]+ [A-Z][a-z]+), (CEO|Founder|Partner|Co-founder|President|Managing Director|Director)", description)
    if m2:
            ...
            for url in urls:
                try:
                    ...
                    if cursor.fetchone() is not None:
                        logger.info("Content ID %s already exists, skipping: %s", content_id, url)
                        continue
...
def _process_single_url(url: str, video_id: str, content_id: str, cursor: sqlite3.Cursor, output_dir: str) -> None:
    ...
    upload_date = f"{upload_date_raw[:4]}-{upload_date_raw[4:6]}-{upload_date_raw[6:]}" if upload_date_raw and len(upload_date_raw) == 8 else None
    ...
    logger.info("Successfully processed YouTube video %s (content_id: %s)", url, content_id)
